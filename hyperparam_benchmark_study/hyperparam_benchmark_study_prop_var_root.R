...
cvind <- 1:5
prop_var_root_mult <- c(0.5, 1, 2, 3, 4, 99)

scenariogrid <- expand.grid(prop_var_root_mult=prop_var_root_mult, cvind=cvind, dataset=datasets, stringsAsFactors = TRUE)
scenariogrid <- scenariogrid[,ncol(scenariogrid):1, drop=FALSE]

set.seed(1234)
seeds <- sample(1000:10000000, size=length(datasets)*length(cvind))

scenariogrid$seed <- rep(seeds, each=length(prop_var_root_mult))
...
save(scenariogrid, file="./intermediate_results/scenariogrid_prop_var_root.Rda")
...
results <- parLapply(cl, 1:nrow(scenariogrid), function(z) {
  try({
    evaluatesetting_prop_var_root(z, current_dir)
  })
})
...
save(results, file="./intermediate_results/results_prop_var_root.Rda")  
...
allfiles <- grep("res_prop_var_root_", list.files("./intermediate_results/"), value=TRUE)

...
cvind <- 1:5
max_depth_root <- c(2, 3, 4, 5, 7)

scenariogrid <- expand.grid(max_depth_root=max_depth_root, cvind=cvind, dataset=datasets, stringsAsFactors = TRUE)
scenariogrid <- scenariogrid[,ncol(scenariogrid):1, drop=FALSE]

set.seed(1234)
seeds <- sample(1000:10000000, size=length(datasets)*length(cvind))

scenariogrid$seed <- rep(seeds, each=length(max_depth_root))
...
save(scenariogrid, file="./intermediate_results/scenariogrid_max_depth_root.Rda")
...
results <- parLapply(cl, 1:nrow(scenariogrid), function(z) {
  try({
    evaluatesetting_max_depth_root(z, current_dir)
  })
})
...
save(results, file="./intermediate_results/results_max_depth_root.Rda")  
...
allfiles <- grep("res_max_depth_root_", list.files("./intermediate_results/"), value=TRUE)
